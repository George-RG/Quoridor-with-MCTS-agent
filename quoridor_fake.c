...
point find_shortest_or_longest_path (board *b,color cur_agent,distance distance);
void compare_paths(int *a,int *b,distance distance);


//MCTS
void MCTS(board *original_board,color player,int sims,listptr* adr, int* move_num);
...
void Heuristic_Move(color cur_agent,board *b);
...
void name(void)
{
	printf("=MCTS quoridor by sdi2100118 and sdi2100161\n");
	fflush(stdout);
}
...
point find_shortest_or_longest_path (board *b,color cur_agent,distance distance)
{
	...
	for(i=0;i<2;i++)
	{
		...
		if(legal_move(b,start,&agent,&oponent))
		{
			...
			compare_paths(current_best,temp_score,distance);
		}
	...
	start.x=current_best[1]; start.y=current_best[0];
	return start;	
}
...
void MCTS(board *original_board,color player,int sims,listptr* adr, int* move_num)
{
	...
	free_board(b);
	tree_node* BestNode = FindBestChild(*root);
...
double GetUCT(int parent_visits,tree_node node)
{
	...
	double temp = ((double)node.wins / (double)node.passes) + sqrt(0.6 * log((double)parent_visits) / (double)node.passes);
	return temp;
}
...
int SimulateRollout(tree_node node,board board)
{
	...
	while (board_winner == NO_PLAYER && heuristic_moves < 30)
	{
		heuristic_moves++;
		current_player = ((current_player)%2)+1; //toggle player

		Heuristic_Move(current_player,&board);
...
void Heuristic_Move(color cur_agent,board *b)
{
	...
	{
		current = find_shortest_or_longest_path(b,cur_agent,SHORTEST);
		move_type=TRANSFER;
	}
	else if(agent.available_walls>0 && (((double)rand() / RAND_MAX) < 0.6) )
	{
	...
	{
		current = find_shortest_or_longest_path(b,cur_agent,LONGEST);
		move_type=TRANSFER;
	}
...
void genmove(board *b, char* buff,int* move_num,listptr *adr)
{
	...
	if(wall_up(p->position,b) && wall_down(p->position,b))
		sad=YES;


	if(p->available_walls>0 && *move_num>3 && !sad)
		MCTS(b,player_color,20000,adr,move_num);
	else
	{
		point current = find_shortest_or_longest_path(b,player_color,SHORTEST);
		addto_undo(TRANSFER,p->player_color,p->position,adr,move_num);
		b->cells[p->position.y][p->position.x].player=NO_PLAYER;
		b->cells[current.y][current.x].player=p->player_color;
		p->position.x=current.x;
		p->position.y=current.y;
